$(document).ready(function(){			
   ...
   // view
	$('.view-modal-data').on('show.bs.modal', function (event) {
		...
		var modal = $(this);
	$.ajax({
		url: site_url+"employees/read_document/",
		type: "GET",
			...
			success: function(JSON)
			{
				if (JSON.error != '') {
					...
					if(user_id == session_id){
						$('.user_profile_avatar').attr("src", JSON.img);
					}
					$('.save').prop('disabled', false);
				}
			},
...
function previewFile() {
  ...
  var file    = document.querySelector('#p_file').files[0];
  var reader  = new FileReader();

  reader.onloadend = function () {
    $('.profile-header-cover').css('background-image', 'url(' + reader.result + ')');
  }
