...
class Expense extends MY_Controller {
    ...
    public function expense_list()
     {
		...
		$data = array();
          ...
          foreach($expense->result() as $r) {
			  ...
			  // get currency
			  $currency = $this->Xin_model->currency_sign(number_format($r->amount,2));
			  // download
			  $download = '';
			  
			  if($r->status==0): $status = '<span class="tag tag-warning">Pending</span>'; elseif($r->status==1): $status ='<span class="tag tag-success">Approved</span>'; elseif($r->status==2): $status = '<span class="tag tag-primary">Paid</span>'; else: $status = 'Cancel'; endif;
	...
	public function add_expense() {
		...
		$remarks = $this->input->post('remarksr');
